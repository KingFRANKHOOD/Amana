...
function getEvidenceMetadataRetentionDays(): number {
  ...
  if (raw !== undefined) {
    ...
    return Number.isFinite(parsed) && parsed > 0
      ? parsed
      : env.EVIDENCE_METADATA_RETENTION_DAYS;
  }
...
export class AuditTrailService {
  constructor(
    private readonly prisma: AuditDatabase = defaultPrisma as unknown as AuditDatabase,
  ) {}

  /**
   * Deterministic serialization of a `CanonicalAuditPayload`.
   *
   * Both signing (sign/verify) and hash computation in the route MUST go
   * through this single helper.  Constructing two separate objects and calling
   * `JSON.stringify` on each can produce different key orderings, making
   * signature verification fail for valid clients.
   *
   * The payload is serialised to a canonical JSON string by iterating keys in
   * the fixed order defined by the `CanonicalAuditPayload` type, so the output
   * is deterministic regardless of the insertion order of the input object.
   */
  serializeCanonicalPayload(payload: CanonicalAuditPayload): string {
    const canonical = {
  ...
  async getTradeHistory(
    ...
    if (
      ["FUNDED", "DELIVERED", "COMPLETED", "DISPUTED", "CANCELLED"].includes(
        trade.status,
      )
    ) {
      events.push({
  ...
  signPayload(payload: CanonicalAuditPayload): AuditIntegrityMetadata {
    ...
    if (!keyId || !privateKeyPem) {
      throw new AuditSigningConfigError(
        "AUDIT_SIGNING_KEY_ID and AUDIT_SIGNING_PRIVATE_KEY_PEM are required",
      );
    }

    const payloadBytes = Buffer.from(
      this.serializeCanonicalPayload(payload),
      "utf8",
    );
  ...
  verifyPayload(
    ...
    if (!publicKeyPem) {
      throw new AuditSigningConfigError(
        "AUDIT_SIGNING_PUBLIC_KEY_PEM is required",
      );
    }
