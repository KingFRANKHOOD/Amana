...
const auditTradeIdParamSchema = z.object({
  id: z
    ...
    .regex(/^[A-Za-z0-9_-]+$/, "Trade ID contains invalid characters"),
});
...
export function createAuditTrailRouter(auditService = new AuditTrailService()) {
  ...
  router.get(
    "/:id/history",
    authMiddleware,
    validateRequest({ params: auditTradeIdParamSchema }),
    async (req: AuthRequest, res: Response) => {
      ...
      try {
        const events = await auditService.getTradeHistory(
          req.params.id as string,
          callerAddress,
        );
        const format = req.query.format as string | undefined;
        const signed = req.query.signed === "true";
        const canonicalPayload = auditService.getCanonicalPayload(
          req.params.id as string,
          events,
        );
        const integrity = signed
          ? auditService.signPayload(canonicalPayload)
          : undefined;
    ...
    async (req: AuthRequest, res: Response) => {
      ...
      try {
        ...
        const payloadHash = crypto
          ...
          .update(
            Buffer.from(
              auditService.serializeCanonicalPayload(canonicalPayload),
              "utf8",
            ),
          )
          .digest("hex");
        ...
        res
          .status(500)
          .json({ error: "Failed to verify trade history signature" });
      }
    },
  );
