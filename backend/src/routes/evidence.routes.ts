...
// Stream uploads to a temporary directory on disk instead of buffering the
// entire file in process memory. The temp file is cleaned up after the
// upload handler resolves (success or failure).
const videoUpload = multer({
  storage: multer.diskStorage({
    destination: (_req, _file, cb) => cb(null, os.tmpdir()),
    filename: (_req, _file, cb) => {
      const unique = `evidence-${Date.now()}-${Math.random().toString(36).slice(2)}`;
      cb(null, unique);
    },
  }),
...
/** Delete a multer disk-storage temp file, ignoring errors (best-effort). */
...
/**
 * Adapt a disk-storage multer file so EvidenceService (which reads
 * `file.buffer`) can consume it.  The file is read from disk once and
 * cached on first access, then the temp file is removed.
 */
function withBuffer(file: Express.Multer.File): Express.Multer.File {
  if (file.buffer && file.buffer.length > 0) return file; // already in-memory
...
export function createEvidenceRouter(evidenceService = new EvidenceService()) {
  ...
  const evidenceUploadLimiter = createWalletRateLimiter(
    RATE_LIMIT_CONFIG.evidenceUpload,
  );
  ...
  router.post(
    ...
    async (req: AuthRequest, res: Response) => {
      ...
      // Wrap disk-storage file so EvidenceService can access file.buffer,
      // then clean up the temp file regardless of outcome.
      const fileWithBuffer = withBuffer(req.file);
